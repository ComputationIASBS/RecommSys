...
# training movie embedder model
model = AutoEncoder(users_features.size(0), embedding_dim=ED, dropout=DO).to(device)
...
for epoch in range(epochs):
    ...
    model.eval()
    x_pred = model(train_rating_matrix.T)
    val_running_loss = masked_rmse_loss(val_rating_matrix, x_pred.T).item()
    ...
    print(f'[{epoch+1}/{epochs}] loss:{running_loss:.4f}, val_loss: {val_running_loss:.4f}')
...
plt.ylabel('Loss')
plt.legend()
plt.title('Auto Encoder Users Embedings RMSE Loss')
plt.show()# Plot train and validation loss
plt.plot(list(range(len(history['train_loss']))), history['train_loss'], label='Train Loss')
plt.plot(list(range(len(history['val_loss']))), history['val_loss'], label='Validation Loss')
...
plt.ylabel('Loss')
plt.legend()
plt.title('Auto Encoder Movies Embedings RMSE Loss')
plt.show()
...
with torch.no_grad():
    movies_embeddings = model.encoder(train_rating_matrix.T)

torch.save(movies_embeddings, f"movies_embeddings_autoencoder_{ED}_{DO}.pt")